"""Step 55 PostgreSQL 连接器测试（V4 多数据源第二落点，ADR-055）。

以 FakeExecutor（惰性替换 handle._executor）驱动，不依赖真实 PG 服务；
凭据净化、setup 序列、schema 类型归一化、内容指纹语义全覆盖。
真实 PG 联调属 integration 标记场景（本地 compose 起库）。
"""
...
from datasentry_core.connectors.postgres import (
    PostgresConnector,
    PostgresDataHandle,
    _RedactingExecutor,
    redact_credentials,
...
_SCHEMA_TABLE = pa.table(
    {
        ...
        "column_type": ["INTEGER", "DECIMAL(10,2)", "TIMESTAMP WITH TIME ZONE"],
    }
)
...
def _handle(
    *,
    ...
    handle._executor = _RedactingExecutor(fake or FakeExecutor(), dsn or "postgresql://***")
    return handle
...
class TestRedactCredentials:
    def test_full_dsn_replaced(self) -> None:
        assert redact_credentials(f"boom {DSN} oops", DSN) == "boom postgresql://*** oops"

    def test_password_replaced(self) -> None:
...
class TestPostgresConnector:
    def test_supports_by_dsn_or_ref(self) -> None:
        ...
        assert (
            connector.supports(
                DataSourceSpec(source_type=DataSourceType.POSTGRESQL, table_name="t")
            )
            is False
        )
        assert (
    ...
    def test_open_without_table_name_raises(self) -> None:
        spec = DataSourceSpec(source_type=DataSourceType.POSTGRESQL, options={"dsn": DSN})
        with pytest.raises(DataSourceNotFoundError):
            PostgresConnector().open(spec)
...
class TestSetupSequence:
    def test_requires_path_false_allows_pathless(self) -> None:
        ...
        assert handle.source_type == DataSourceType.POSTGRESQL

    def test_ensure_view_sequence(self) -> None:
        fake = FakeExecutor(responses={"DESCRIBE": _SCHEMA_TABLE})
        handle = _handle(fake=fake)
        handle._ensure_view()
        assert "LOAD postgres" in fake.calls
        assert any(c.startswith("ATTACH 'postgresql://user:") for c in fake.calls)
        assert any(c.startswith('SELECT 1 FROM pg."public"."payments"') for c in fake.calls)
        assert any(
            c.startswith('CREATE OR REPLACE VIEW data AS SELECT * FROM pg."public"."payments"')
            for c in fake.calls
        )

    def test_ensure_view_custom_schema_identifier_escaped(self) -> None:
        fake = FakeExecutor(responses={"DESCRIBE": _SCHEMA_TABLE})
        handle = _handle(schema='we"ird', fake=fake)
        handle._ensure_view()
        assert any('"we""ird"' in c for c in fake.calls)
    ...
    def test_extension_load_failure_hints(self) -> None:
        fake = FakeExecutor(
            responses={"DESCRIBE": _SCHEMA_TABLE},
            fail_on={"LOAD postgres": duckdb.Error("extension not bundled")},
        )
        handle = _handle(fake=fake)
        with pytest.raises(ConnectorError) as exc:
            handle._ensure_view()
        assert "failed to load duckdb postgres extension" in str(exc.value)

    def test_attach_failure_hints_and_redacts(self) -> None:
        fake = FakeExecutor(
            responses={"DESCRIBE": _SCHEMA_TABLE},
            fail_on={"ATTACH": duckdb.Error(f"connect refused {DSN}")},
        )
        handle = _handle(fake=fake)
        with pytest.raises(ConnectorError) as exc:
            handle._ensure_view()
        message = str(exc.value)
        assert "postgres connection failed (check host/port/database/credentials)" in message
    ...
    def test_table_not_found(self) -> None:
        ...
        with pytest.raises(DataSourceNotFoundError) as exc:
            handle._ensure_view()
        assert "public.payments" in str(exc.value)
...
class TestSchema:
    ...
    def test_ensure_view_called_once(self) -> None:
        ...
        handle.schema()
        handle.schema()
        assert fake.calls.count("LOAD postgres") == 1
