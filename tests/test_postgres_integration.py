...
class TestPgIntegration:
    ...
    def test_scheduler_skip_on_unchanged_real_pg(
        ...
        try:
            ...
            con.execute(f"ATTACH '{pg_dsn}' AS w (TYPE postgres)")
            con.execute(
                f"INSERT INTO w.public.{orders_table} (order_id, customer_email, "
                "status, order_total) VALUES (999, 'ins@x.com', 'pending', 9.99)"
            )
        finally:
            con.close()
        fourth_id = scheduler.trigger("job_pg_it")
    ...
    @pytest.mark.integration
    def test_stats_layer_semantics_real_pg(self, pg_dsn: str, orders_table: str) -> None:
        """Step 58 统计层语义（真实 PG）：原地 UPDATE（行数不变）不移动统计层，
