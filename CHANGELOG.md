...
- Web 批量结果视图：批量扫描完成跳转扫描列表，顶部汇总横幅
  （文件数/问题数/平均分，失败时橙色列出失败文件与原因）；
  扫描列表每行新增六维迷你条（悬停显示维度分数，色同趋势图）。
- Web 批量部分失败语义：缺失/失败文件不中断扫描，结果横幅
  展示；全部失败仍 404 错误页（逐文件原因）。
- 性能基准脚本 scripts/benchmark.py：本地可复现端到端扫描
  计时（10k/100k/300k 行脏数据），输出 markdown 表。
...
- 回滚移出修复页主操作按钮行，独立成"危险区"（红色边框 +
  说明文字），与 propose/preview/apply 视觉隔离。
- 回滚增加二次确认弹窗：说明回滚作用（撤销最近一次 apply、
  恢复修复前状态、不可逆），需明确点击"确认回滚"才执行。
- 确认弹窗默认焦点落在"取消"按钮（回车=取消），必须主动
  确认才执行——退出确认同样处理，统一防误触语义。



### 修复（TUI 真实终端可用性，ADR-118 续）

- 扫描完成通知改用全局 `notify()`：原完成信息写入扫描页
  状态区，但在下一帧渲染前即切换到问题页，用户看不到
  结果；现跨视图可见（含评分与问题数）。
- 数字键切换视图后焦点进入目标控件（扫描页输入框 / 问题
  列表 / 修复页输入框），避免输入被焦点吞掉。
- 扫描页输入框回车直接触发扫描（`Input.Submitted`），无需
  再点击按钮。
...
### 新增（Step 97，API 并行配置，ADR-097）

- **DATASENTRY_MAX_WORKERS** 环境变量：>1 时调度异步派发并行
  执行；默认/非法回退 1（同步语义零迁移）
- **app.state.scheduler** 暴露（运维/测试观测）；服务退出优雅
